...
async def recognitionLoop(recoResult, ws):
    ...
    last_recognition_time = 0  # 上一次辨識的時間戳
    ...
    try:
        while True:
            ...
            # 每隔固定時間（例如 1 秒）進行一次辨識
            if time.time() - last_recognition_time > 1:  # 每 1 秒進行一次辨識
